...
import { StackArrayImpl as Stack } from './../../../koans/ds/stack/index.js';



const MSG = "Stack array implementation FAIL!\n"
+ "See file: ds/stack/001-array-implementation.js";
...
function test(){
  ...
  assert.deepStrictEqual(test1, 30, MSG);
  assert.deepStrictEqual(s.peek(), 20, MSG);
  assert.deepStrictEqual(s.isEmpty(), false, MSG);
  assert.deepStrictEqual(s.toString(), "20 10", MSG);
  ...
  assert.deepStrictEqual(s.isEmpty(), false, MSG);
  assert.deepStrictEqual(s.toString(), "40 10", MSG);
  ...
  assert.deepStrictEqual(test4, 10, MSG);
  assert.deepStrictEqual(s.peek(), null, MSG);
  assert.deepStrictEqual(s.isEmpty(), true, MSG);
  ...
  assert.deepStrictEqual(test5, null, MSG);
  assert.deepStrictEqual(s.peek(), null, MSG);
  assert.deepStrictEqual(s.isEmpty(), true, MSG);
  assert.deepStrictEqual(s.toString(), "EMPTY", MSG);
